...
def create_app():
    ...
    export_options_example = os.path.join(app.root_path, "export_options.example.json")
    if not os.path.exists(export_options_path) and os.path.exists(export_options_example):
        try:
            shutil.copyfile(export_options_example, export_options_path)
        except OSError as exc:
            app.logger.warning("Não foi possível copiar export_options.example.json: %s", exc)
    ...
    def _after_commit(session):
        ...
        if not app.config.get("BACKUP_CREATED_THIS_SESSION"):
            app.config["BACKUP_CREATED_THIS_SESSION"] = _backup_database()
    ...
    def _backup_database():
        ...
        backup_dir = app.config.get("BACKUP_DIR") or os.path.join(
            app.instance_path, "backups"
        )
